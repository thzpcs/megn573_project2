        ...
        for file in files:
            ...
            # to the specific joints, with x, y, and z position 
                ...
                if data[1] == 1:
                    ...
                    histD1 = np.histogram(d1, bins=35, range= dRange)
                    
                    # Uses the largest number of bins (d1), which determines the bins for all other dHist
                    nBins = len(histD1[0])
                    
                    histD2 = np.histogram(d2, bins=nBins, range= dRange)
                    histD3 = np.histogram(d3, bins=nBins, range= dRange)
                    histD4 = np.histogram(d4, bins=nBins, range= dRange)
                    histD5 = np.histogram(d5, bins=nBins, range= dRange)
                            
                    histTheta1 = np.histogram(theta1, bins=15, range= thetaRange)
                    
                    mBins = len(histTheta1[0])
                    
                    histTheta2 = np.histogram(theta2, bins=mBins, range= thetaRange)
                    histTheta3 = np.histogram(theta3, bins=mBins, range= thetaRange)
                    histTheta4 = np.histogram(theta4, bins=mBins, range= thetaRange)
                    histTheta5 = np.histogram(theta5, bins=mBins, range= thetaRange)
                    ...
                    norm_d1 = preprocessing.normalize(instance_d1, norm='l2')
                    
            rad_d1 = np.vstack([rad_d1,norm_d1])
            #print(str(trainData))
            trainData.close()
...
def HJPD(filePath):
    ...
    for subdir, dirs, files in os.walk(rootDir):
        for file in files:
            ...
            instance_hjpd_d1 = np.histogram(deltaFrame, bins=100, range= dRange)
            
            
                    
            hjpd_d1 = np.vstack([hjpd_d1,instance_hjpd_d1[0]])
                        
            norm_hjpd_d1 = preprocessing.normalize(hjpd_d1, norm='l2')
...
def convertToSVM(convertFile, filePath):
        ...
        for newLine in dataFile:
            ...
            for index in range(len(data)):
                ...
                svmLine = svmLine + str(index+1) + ':' + str(data[index]) + ' '
            svmLine = svmLine + '   ' +  '\n' +  str(activity[act]) + '    '
